(__createModule('jetpack', function($F) {
    var __self__ = this; // reference to module
    var constructor = (/*
 * jetpack (js)
...
function (options) {
    ...
    function setDuration(duration) {
        ...
        if (duration) {
            if (!isNaN(duration)) globalDuration = Number(duration);else if (DEFAULT_DURATIONS.ifhasOwnProperty(optionDuration)) globalDuration = DEFAULT_DURATIONS[duration];else __self__.warn('invalid parameter: "' + duration + '" keeping default');
        }
    };

    // toggle for whether the location href should update
    function setupdateURL(value) {
        if (typeof value === 'boolean') updateURL = value;else __self__.warn('invalid parameter: "' + value + '" keeping default');
    };

    // toggle for whether animations should be enabled or disabled
    function setAnimate(value) {
        if (typeof value === 'boolean') animationEnabled = value;else __self__.warn('invalid parameter: "' + value + '" keeping default');
    };
